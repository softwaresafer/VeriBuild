...
from VeMakeCDG.CDGninja import get_CDG_ninja
from VeMakeDDG.DDGninja import get_DDG_ninja
...
def singlerun(filepath, outputdir, execmethod, cleanmethod, touchmethod, detectIssue, jnum, isSimplified):
    ...
    DDG = DDG_filler(CDG, DDG, fileIndexMap) # TODO: ERORR PRONE
    print("DDG filler finished")
    GDDG = get_GDDG(CDG, DDG)
    print("GDDG construction finished")
    restb = get_resourceTable_ninja(CDG, DDG, GDDG, fileIndexMap)
    ...
    # echotime(pre_time, CDG_time, DDG_time, res_time, MP_time, EP_time, FT_time, FR_time, IC_time, total_time,
    #          outputpath)
    ...
    # VMReport_time = generate_reportor_ninja(filepath, outputpath, touchmethod == "-touch", detectIssue, MP_fileMap, EP_fileMap)
    #
    # # compare with mkcheck
    # # compare_run(project_name, MP_result, EP_result, shortDDG)
    return
...
#         os.system(
#             "pp-report --remote localhost --remote-port 40088 --upload " + reportname + " --user adminuser --passwd vemaketest --builddir . --allow-invalid-types ../../testoutput/" + project_name + "/vmreport.json")
...
if __name__ == "__main__":
    ...
    if (not validoption):
        print("option error")
    elif ((not (set(filter(lambda op: op.startswith("-"), sys.argv[1:]))).issubset(set(
            ["-j", "-simple", "-config", "-batch", "-f", "-configure", "-onlytouch", "-touch", "-trace", "-cache",
             "-MP", "-EP", "-FT", "-FR", "-IC"]))) or (("-config" in sys.argv[1:]) and ("-configure" in sys.argv[1:]))):
        print("option error")
    else:
        if ("-f" in sys.argv[1:]):
        ...
        for filepath in filepath_list:
            ...
            if "-onlytouch" in sys.argv[1:]:
                ...
                #     batchrun(filepath, outputdir, execmethod, cleanmethod, touchmethod, detectIssue, jnum,
                #              "-simple" in sys.argv[1:])
                # else:
                singlerun(filepath, outputdir, execmethod, cleanmethod, touchmethod, detectIssue, jnum, "-simple" in sys.argv[1:])
